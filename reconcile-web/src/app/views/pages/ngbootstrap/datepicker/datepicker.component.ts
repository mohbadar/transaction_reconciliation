...
<div class="kt-section">
  <div class="kt-section__content">
    <pre>From: {{ openTime | json }} </pre>
    <pre>To: {{ closeTime | json }} </pre>
  </div>
</div>
`,
	tsCode: `
...
export class NgbdDatepickerRange {
  ...
  openTime: NgbDateStruct;
  closeTime: NgbDateStruct;\n
  constructor(calendar: NgbCalendar) {
    this.openTime = calendar.getToday();
    this.closeTime = calendar.getNext(calendar.getToday(), 'd', 10);
  }\n
  onDateChange(date: NgbDateStruct) {
    if (!this.openTime && !this.closeTime) {
      this.openTime = date;
    } else if (this.openTime && !this.closeTime && after(date, this.openTime)) {
      this.closeTime = date;
    } else {
      this.closeTime = null;
      this.openTime = date;
  ...
  isHovered = date => this.openTime && !this.closeTime && this.hoveredDate && after(date, this.openTime) && before(date, this.hoveredDate);
  isInside = date => after(date, this.openTime) && before(date, this.closeTime);
  isFrom = date => equals(date, this.openTime);
  isTo = date => equals(date, this.closeTime);
}
`,
	isCodeVisible: false,
	isExampleExpanded: true
